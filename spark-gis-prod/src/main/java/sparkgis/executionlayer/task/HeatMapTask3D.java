...
import sparkgis.executionlayer.SparkPrepareData3D;
import sparkgis.executionlayer.SparkSpatialJoinHM_Cogroup;

public class HeatMapTask3D extends Task implements Callable<String>{
    ...
    public HeatMapTask3D(String jobId,
	...
	DataConfig3D[] configs = new DataConfig3D[algoCount];
	List<Future<DataConfig3D>> futures = new ArrayList<Future<DataConfig3D>>();
	for (int i=0; i<algoCount; ++i){
	    //futures.add(exeService.submit(new AsyncPrepareData(super.data, algos.get(i), super.inputSrc)));
	    futures.add(exeService.submit(new AsyncPrepareData3D(super.data,
							       algos.get(i))));
	}
	try{
	    for (int i=0; i<algoCount; ++i)
     ...
     */
    public class AsyncPrepareData3D implements Callable<DataConfig3D>{
        private final String caseID;
    	private final String algo;	
    	public AsyncPrepareData3D(String caseID, String algo){
    	    this.caseID = caseID;
    	    this.algo = algo;    	    
    	}
	
    	@Override
    	public DataConfig3D call(){
    		...
    		SparkPrepareData3D job = new SparkPrepareData3D(caseID);
    		DataConfig3D ret = job.execute(polygonsRDD);
    		return ret;
    	    }
    	    return null;
    	}
